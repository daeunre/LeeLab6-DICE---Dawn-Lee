...
function letsPlay() {
  numRolls = prompt("How many times would you like to roll the dice?");
  numRolls = Number(numRolls);
  
  while (isNaN(numRolls) || numRolls < 1 || numRolls > 50) { 
    numRolls = (prompt("Please enter a valid number between 1 and 50:"));
  }
  ...
  results += `${i+1}. ${die1} + ${die2} = ${dieSum} -> ${outcome} <br>`;
}

document.querySelector("h3").innerHTML = 
  `<div id="resultsBox">
    <strong>Roll Results:</strong><br>${results}
  </div>
    <br><strong>Final Balance: $${balance}</strong>`;
